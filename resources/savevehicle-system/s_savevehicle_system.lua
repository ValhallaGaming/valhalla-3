...
function saveVehicle(source)
	...
	if isElement(source) and getElementType(source) == "vehicle" and dbid >= 0 then -- Check it's a permanently spawned vehicle and not a job vehicle
		...
		if (owner~=-1) then
			...
			--local tintedwindows = getElementData(source, "tinted") or 0
			
			local dimension = getElementDimension(source)
			local interior = getElementInterior(source)
			
			mysql:query_free("UPDATE vehicles SET panel0='" .. mysql:escape_string(panel0) .. "', panel1='" .. mysql:escape_string(panel1) .. "', panel2='" .. mysql:escape_string(panel2) .. "', panel3='" .. mysql:escape_string(panel3) .. "', panel4='" .. mysql:escape_string(panel4) .. "', panel5='" .. mysql:escape_string(panel5) .. "', panel6='" .. mysql:escape_string(panel6) .. "', door1='" .. mysql:escape_string(door1) .. "', door2='" .. mysql:escape_string(door2) .. "', door3='" .. mysql:escape_string(door3) .. "', door4='" .. mysql:escape_string(door4) .. "', door5='" .. mysql:escape_string(door5) .. "', door6='" .. mysql:escape_string(door6) .. "', hp='" .. mysql:escape_string(health) .. "', sirens='" .. mysql:escape_string(sirens) .. "', currdimension='" .. mysql:escape_string(dimension) .. "', currinterior='" .. mysql:escape_string(interior) .. "', Impounded='" .. mysql:escape_string(tonumber(Impounded)) .. "', handbrake='" .. mysql:escape_string(tonumber(handbrake)) .. "' WHERE id='" .. mysql:escape_string(dbid) .. "'")
			 -- , tintedwindows='" .. mysql:escape_string(tonumber(tintedwindows)) .. "'
...
function saveVehicleMods(source)
	...
	if isElement(source) and getElementType(source) == "vehicle" and dbid >= 0 and owner > 0 then
